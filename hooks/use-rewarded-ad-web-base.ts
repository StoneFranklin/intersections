...
/**
 * Generic base hook for managing rewarded ads on web platform.
 * Uses Google AdSense Ad Placements API (adBreak) for rewarded ads.
 * Falls back to simulation if AdSense is not available.
 *
 * @param placementName - The name of the ad placement (e.g., "extra-life", "double-xp")
 */
export function useRewardedAdWebBase(placementName: string): UseRewardedAdReturn {
        ...
        if (typeof window !== 'undefined' && (window as any).adBreak) {
          ...
          adBreak({
            ...
            adDismissed: () => {
              ...
              resolved = true;
              setIsShowing(false);
              console.log(`AdSense: ${placementName} ad dismissed before completion`);
            ...
            adViewed: () => {
              ...
              resolved = true;
              setIsShowing(false);
              console.log(`AdSense: ${placementName} ad viewed - Reward granted!`);
          ...
          console.log(`AdSense not available, simulating ${placementName} rewarded ad`);
          setIsLoading(true);
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              setIsShowing(false);
              console.log(`Simulated ${placementName} ad completed - Reward granted!`);
              resolve({ success: true, rewarded: true });
            }, 3000);
          }, 500);
        }
      } catch (err) {
        console.error(`Web ${placementName} ad error:`, err);
        setIsLoading(false);
        setIsShowing(false);
